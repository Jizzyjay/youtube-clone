...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${YOUTUBE_API_KEY}/videos?key=${API_KEY}&part=snippet,statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  const {
    ...
    `${YOUTUBE_API_KEY}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`
  );
